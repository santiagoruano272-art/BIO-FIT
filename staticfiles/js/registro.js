...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (gymSelect) {
        gymSelect.addEventListener('change', () => {
            if (homeBadge) homeBadge.classList.toggle('visible', gymSelect.value === 'home');
        });
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!regRes.ok) {
                showAlert(regData.error || 'Error al crear la cuenta.');
                return;
            }
            ...
            if (loginRes.ok && loginData.token) {
                localStorage.setItem('biofit_token', loginData.token);
                localStorage.setItem('biofit_uid', loginData.uid);
            }

            const destino = gym_id === null
                ? '/login/'
                : '/inventory/dashboard/';
